...
class SystemShare(models.Model):
    ...
    shared_by = models.ForeignKey(
        User, on_delete=models.PROTECT, related_name="shared_systems"
    )
    shared_with = models.ManyToManyField(
        User, related_name="shared_systems_with_me", blank=True
    )
...
class Activity(StarterModel):
    ...
    activity_name = models.TextField(verbose_name="Faaliyet")
...
class ActivityAttachment(models.Model):
    ...
    file_size = models.PositiveIntegerField(
        verbose_name="Dosya Boyutu (bytes)", default=0
    )
